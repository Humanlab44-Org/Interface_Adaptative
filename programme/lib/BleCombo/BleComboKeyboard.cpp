...
#define SERVICE_UUID "689cb666-4b7e-4913-9600-41ec4408a8aa"
...
void BleComboKeyboard::disconnectAll(void)
{
	BLECharacteristic *pNotifyCharacteristic;
	if (this->pServer->getConnectedCount() > 0)
	{
		BLEDevice::stopAdvertising();
		Serial.println(F("Deconnexion du client BLE"));

		Serial.println(this->pServer->getConnectedCount());
		uint16_t conn_id = this->pServer->getConnId();
		this->pServer->disconnect(conn_id); // ca c'est OK
		this->pServer->removePeerDevice(conn_id, true);
		// ---------------------------------------------------------------------
		// TODO    empecher la reconnexion automatique
		//  ----------------------------------------------------------------
		BLEDevice::startAdvertising();
	}
	else
	{
		Serial.println(F("Aucun client BLE connecté"));
	}
}
...
void BleComboKeyboard::taskServer(void *pvParameter)
{
	...
	//  initialise et configure les fonctionnalités HID du périphérique BLE
	...
	// Mise en place de la fonction de callback pour la gestion des connexions
	...
	// Configuration des informations HID
	bleKeyboardInstance->hid->pnp(0x02, 0xe503, 0xa112, 0x0210);
	// bleKeyboardInstance->hid->hidInfo(0x00, 0x01); // 0x00 : Numéro de version HID  0x01 : Type de pays (Country Code).
	bleKeyboardInstance->hid->hidInfo(0x00, 0x01); // version 00 et  pas de code pays specifié (0x05 est la france, 0x01 USA)
	...
	// Configuration du report map et start services
